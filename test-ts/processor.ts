...
describe('Processor', () => {
    ...
    describe('use()', () => {
        ...
        it('throws on wrong format', () => {
            ...
            expect( () => pr.use(<any>1) ).to.throw(/1 is not a PostCSS plugin/);
        });
    ...
    describe('process()', () => {
        ...
        let beforeFix = new Processor([ css => {
            css.walkRules( rule => {
                ...
                if ( !rule.some( i => (<Declaration>i).prop === 'content' ) ) {
        ...
        it('checks plugin compatibility', () => {
            ...
            expect((<any>console.warn).callCount).to.eql(1);
            expect((<any>console.warn).args[0][0]).to.eql(
                'Your current PostCSS version is 1.0.0, but test uses 2.1.5. ' +
                'Perhaps this is the source of the error below.');

            expect( () => processBy('3.0.0') ).to.throws('Er');
            expect((<any>console.warn).callCount).to.eql(2);

            expect( () => processBy('2.0.0') ).to.throws('Er');
            expect((<any>console.warn).callCount).to.eql(3);
