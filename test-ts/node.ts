...
describe('Node', () => {
    ...
    describe('error()', () => {
        ...
        it('highlights index', () => {
            ...
            let error = (<Rule>root.first).first.error('Bad semicolon', { index: 1 });
        ...
        it('highlights word', () => {
            ...
            let error = (<Rule>root.first).first.error('Wrong color', { word: 'x' });
            expect(error.showSourceCode(false)).to.eql('\n' +
                                                       'a { color: x red }\n' +
                                                       '           ^');
        });
    ...
    describe('warn()', () => {
        ...
        it('accepts options', () => {
            ...
            let result = postcss([ warner() ]).process('a{}');
            expect(result.warnings().length).to.eql(1);
            expect((<any> result.warnings()[0]).index).to.eql(1);
        });
    ...
    describe('replaceWith()', () => {
        ...
        it('replaces node', () => {
            ...
            let result = <Declaration>(<Rule>css.first).first.replaceWith(decl);
    ...
    describe('next()', () => {
        ...
        it('returns next node', () => {
            let css = parse('a{one:1;two:2}');
            expect((<Rule>css.first).first.next()).to.equal((<Rule>css.first).last);
            expect((<Rule>css.first).last.next()).to.not.exist;
        });
    ...
    describe('prev()', () => {
        ...
        it('returns previous node', () => {
            let css = parse('a{one:1;two:2}');
            expect((<Rule>css.first).last.prev()).to.equal((<Rule>css.first).first);
            expect((<Rule>css.first).first.prev()).to.not.exist;
        });
    ...
    describe('moveTo()', () => {
        ...
        it('moves node inside one root', () => {
            let css = parse('a{\n one:1}\n@page {\n b {\n  two: 2\n }\n}');
            css.first.moveTo(<Rule>css.last);
    ...
    describe('moveBefore()', () => {
        ...
        it('moves node inside one root', () => {
            let css = parse('a{\n one:1}\n@page {\n b {\n  two: 2\n }\n}');
            css.first.moveBefore((<Rule>css.last).first);
    ...
    describe('moveAfter()', () => {
        ...
        it('moves node inside one root', () => {
            let css = parse('a{\n one:1}\n@page {\n b {\n  two: 2\n }\n}');
            css.first.moveAfter((<Rule>css.last).first);
    ...
    describe('root()', () => {
        ...
        it('returns root', () => {
            ...
            expect((<Rule>(<AtRule>css.first).first).first.root()).to.equal(css);
        });
    ...
    describe('cleanRaws()', () => {
        ...
        it('cleans style recursivelly', () => {
            let css = <Root>parse('@page{a{color:black}}');
            css.cleanRaws();

            expect(css.toString())
                .to.eql('@page {\n    a {\n        color: black\n    }\n}');
            expect(css.first.raws.before).to.not.exist;
            expect((<Rule>(<AtRule>css.first).first).first.raws.before).to.not.exist;
            expect(css.first.raws.between).to.not.exist;
            expect((<Rule>(<AtRule>css.first).first).first.raws.between).to.not.exist;
        ...
        it('keeps between on request', () => {
            ...
            expect((<Rule>(<AtRule>css.first).first).first.raws.between).to.exist;
            expect(css.first.raws.after).to.not.exist;
        });
    ...
    describe('positionInside()', () => {
        ...
        it('returns correct position when node starts mid-line', () => {
            ...
            let one = <Declaration>(<Rule>css.first).first;
